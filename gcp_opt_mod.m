function [M, M0, info] = gcp_opt_mod(X, A_gt,nc, varargin)
...
% add by wenqiang
params.addParameter('datatype', [], @ischar);
params.addParameter('ftype', [], @ischar);
...
params.addParameter('maxiters', 30, @isscalar);
...
% =====================  add by wenqiang =====================
datatype = params.Results.datatype;
ftype = params.Results.ftype;
% =====================  add by wenqiang =====================
...
if use_stoc
    ...
    if strcmp(gsampler_type, 'semi-stratified') || strcmp(gsampler_type, 'stratified')
        ...
        if strcmp(gsampler_type, 'semi-stratified')
            ...
            if isempty(xnzidx)
%                 xnzidx = tt_sub2ind64(sz,X.subs);
%             % modified by Wenqiang
                ind = find(X~=0);
                [I1,I2,I3] = ind2sub(sz,ind);
                xnzidx = tt_sub2ind64(sz,[I1,I2,I3]);
                xnzidx = sort(xnzidx);
            end
...
%     P = ktensor(M0);
%     fest_trace(1)  = -tt_loglikelihood(X,P)/(1);%(err_e);
    fest_trace(1) = fest;
    % =====================  added by wenqiang =====================
    Mest = cpdgen(M.U);
    Mgt = cpdgen(A_gt);
    fval = lossfunc(double(X), Mest, ftype);


    fval_trace = zeros(maxiters+1,1);
    mse_t_trace  = zeros(maxiters+1,1);
    mse_a_trace  = zeros(maxiters+1,1);
    
    mse_a_trace(1) = mseA( M.U, A_gt);
    mse_t_trace(1) = mseT( Mgt, Mest, datatype );
    fval_trace(1) = fval;
    
    tt0 = tic;
    time_trace(1) = toc(tt0);
...
%              t_count = tic;
            ...
            % add by wenqiang
%             CoutTime.SampG(titers) = toc(t_count);
...
%              t_count = tic;
            ...
            % add by wenqiang
%             CoutTime.CompG(titers) = toc(t_count);
...
%              t_count = tic;
            ...
            % add by wenqiang
%             CoutTime.UpdataA(titers) = toc(t_count);
        end
        
        
            
        % =====================  added by wenqiang =====================
        time_trace(nepoch+1) = time_trace(nepoch) + toc(tt0); 
        % =====================  added by wenqiang =====================
        ...
        Mgt = cpdgen(A_gt);
        fval = lossfunc(double(X),Mest,ftype);
        mse_t_trace(nepoch+1) = mseT( Mgt, Mest, datatype );
        mse_a_trace(nepoch+1) = mseA( M.U, A_gt);
        fval_trace(nepoch+1) = fval;
        tt0 = tic;
        % =====================  added by wenqiang =====================
        ...
        if (printitn > 0) && (mod(nepoch,printitn) == 0 || failed_epoch || festtol_test)
            fprintf('Epoch %2d: f-est = %e, mse = %e, step = %g', nepoch, fest, step);
            if failed_epoch
    ...
    info.mse_t_trace  = mse_t_trace(1:nepoch+1);
    info.mse_a_trace  = mse_a_trace(1:nepoch+1);
    info.fval_trace  = fval_trace(1:nepoch+1);
    info.nepoch = nepoch;
    info.CoutTime = CoutTime;
    if printitn > 0
        fprintf('End Main Loop\n');
        fprintf('\n');
